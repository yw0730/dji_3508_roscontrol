...
constexpr int16_t kCurrentLimit = 16384;
}  // namespace
...
bool DjiM3508Hardware::parse_feedback_frame(const can_frame & frame)
{
  ...
  hw_velocities_[0] = static_cast<double>(raw_rpm) * kTwoPi / 60.0;
...
int16_t DjiM3508Hardware::compute_current_command(double dt_sec)
{
  ...
  const double error = hw_commands_[0] - hw_velocities_[0];
  ...
  const double clamped = std::clamp(
    current,
    -static_cast<double>(kCurrentLimit),
    static_cast<double>(kCurrentLimit));
